...
class DBBdonnerManager{
  ...
  Future openDB() async {
    _database = await openDatabase(join(await getDatabasesPath(), "bdonner.db"),
        version: 1, onCreate: (Database db, int version) async {
          await db.execute("CREATE TABLE Bdonner(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT,age TEXT,gender Text,bgroup TEXT,phone TEXT,email TEXT)");
        });
  }
  ...
  Future<List<Bdonner>> getBdonnerList() async {
    ...
    return List.generate(maps.length, (index) {
      return Bdonner(id: maps[index]['id'], name: maps[index]['name'], age: maps[index]['age'],gender: maps[index]['gender'],bgroup: maps[index]['bgroup'],
...
class Bdonner{
  int? id;
  late String name,age,bgroup,phone,email,gender;
  Bdonner({this.id,required this.name,required this.bgroup,required this.age,required this.phone,required this.email,required this.gender});
  Map<String,dynamic> toMap(){
    return{'name':name,'bgroup':bgroup,'age':age,'phone':phone,'email':email,'gender':gender};
